...
Usage:
    ...
    $ python yolo_segment/val.py --weights yolov5s-seg.pt --data coco.yaml --img 640  # validate COCO-segments

Usage - formats:
    $ python yolo_segment/val.py --weights yolov5s-seg.pt                 # PyTorch
...
def run(
    ...
    if training:  # called by train.py
        ...
        if engine:
            ...
            device = model.device
            if not (pt or jit):
                batch_size = 1  # yolo_export.py models default to batch-size 1
...
def main(opt):
    check_requirements(requirements=ROOT / 'yolo_requirements.txt', exclude=('tensorboard', 'thop'))
